...
class AppModule(BaseModule):
    ...
    # region pytanieAppDialog
        ...
        self.pytanieAppDialog.instruction_lbl.setText('<html><head/><body><p><span style=" font-weight:600;">APP</span> – obejmuje dane przestrzenne dla jednego aktu planowania przestrzennego np. dla wybranego miejscowego planu zagospodarowania przestrzennego. </p><p><span style=" font-weight:600;">Zbiór APP</span> – obejmuje zestaw danych przestrzennych dla wielu (co najmniej jednego) aktów planowania przestrzennego tego samego rodzaju, występujących w danej jednostce podziału terytorialnego np. miejscowe plany zagospodarowania przestrzennego danej gminy. </p><p>Szczegółowe informacje nt. zakresu danych ujętych w zbiorze można znaleźć w specyfikacji danych. </p><p>Aby utworzyć zbiór APP, należy wcześniej przygotować pojedyncze pliki GML dla każdego APP, który zostanie włączony do tego zbioru. </p><p>Dane przestrzenne dla jednego APP obejmują: </p><p><span style=" font-family:\'Symbol\';">- </span>rysunek lub rysunki APP, w postaci plików rastrowych z nadaną georeferencją, w formacie GeoTIFF, wraz z informacją o adresie internetowym pod którym jest on opublikowany; </p><p><span style=" font-family:\'Symbol\';">- </span>granicę obszaru objętego APP określoną w układzie PL-1992 (EPSG:2180) lub PL-2000 (EPSG:2176, 2177, 2178, 2179), posiadającą reprezentację geometryczną w postaci jednego poligonu lub multipoligonu (w przypadku obiektów wieloczęściowych);</p><p><span style=" font-family:\'Symbol\';">- </span>informacje nt. dokumentów formalnych powiązanych z danym APP (np. uchwała o przystąpieniu do sporządzenia APP, czy uchwała uchwalająca APP). </p></body></html>')
    # endregion
